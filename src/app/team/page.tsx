...
const bgList = ['#eab308','#10b981','#06b6d4','#ef4444','#a855f7','#f97316','#f1f5f9']
            ...
            <div className="border rounded-md">
                {members.length === 0 ? (
                    <Alert >
                        ...
                        <AlertDescription>
                          There is no users currently online!
                        </AlertDescription>
                      </Alert>
                ): null}
                {members &&
                    members.map((member, key) => (<div className="grid grid-cols-6 pr-4 items-center justify-between border-b last:border-0" key={key}>
                        ...
                        <div className="col-span-3 flex gap-5">
                            <Badge className={`tag ${member.roleid}`}>
                                {member.role}
                            </Badge>
                            {member.status && <Badge className={`tag ${member.statusid}`}>
                                {member.status}
                            </Badge>}
                        </div>
